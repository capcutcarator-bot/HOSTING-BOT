...
def start_project(proj_id, data):
    ...
    if railway_environment():
        for other_id, other in data["projects"].items():
            if other_id != proj_id and is_alive(other.get("pid")) and other.get("port") == assigned_port:
                return False, "Railway exposes one public port per service. Stop the other API project before starting this one."
    logf = open(log_path(proj_id), "w")
    env = os.environ.copy()
    env["PYTHONUNBUFFERED"] = "1"
    env["PORT"] = str(assigned_port)
    env["API_PORT"] = str(assigned_port)
    env["HOST"] = "0.0.0.0"
    try:
    ...
    detected_port = wait_for_api_port(proc.pid, assigned_port)
    proj["pid"] = proc.pid
    proj["pgid"] = proc.pid  # since setsid, pgid == pid
    proj["started_at"] = time.time()
    proj["runtime"] = runtime
    proj["command"] = command
    proj["port"] = detected_port
    proj["local_url"] = f"http://127.0.0.1:{detected_port}" if detected_port else None
    proj["public_url"] = None
    if detected_port and PUBLIC_URL_TEMPLATE:
        proj["public_url"] = PUBLIC_URL_TEMPLATE.format(port=detected_port)
    elif detected_port and railway_public_url():
        proj["public_url"] = railway_public_url()
    proj["tunnel_pid"] = None
    if detected_port and not proj["public_url"]:
        proj["public_url"], proj["tunnel_pid"] = start_cloudflared_tunnel(detected_port)
    save_data(data)
    url_note = f" URL: {proj['public_url'] or proj['local_url']}" if detected_port else ""
    return True, f"Started (PID {proc.pid}).{url_note}"


def stop_project(proj_id, data):
    proj = data["projects"][proj_id]
    stop_cloudflared_tunnel(proj)
    pid = proj.get("pid")
    if not is_alive(pid):
        proj["pid"] = None
        save_data(data)
...
def project_ctrl_kb(pid, data):
    ...
    if proj.get("local_url") or proj.get("public_url"):
        kb.add(styled_button("🌐 URL", callback_data=f"url:{pid}"))
...
def register_project_folder(chat_id, folder, name, source=None):
    ...
    url_note = f"\n{project_url_text(data['projects'][proj_id])}" if run_ok and project_url_text(data['projects'][proj_id]) else ""
    bot.send_message(chat_id, f"✅ <b>Project imported and processed</b>\n<blockquote>{html.escape(name)}</blockquote>\n{entry_note}{bot_name}\n\n{dep_note}{run_note}{url_note}", reply_markup=project_ctrl_kb(proj_id, data))
...
def handle_callback(c):
    ...
    if action == "home":
        ...
        if cmd == "open":
            running = is_alive(proj.get("pid"))
            url_note = project_url_text(proj)
            ...
            url_note = project_url_text(proj)
            bot.answer_callback_query(c.id)
            bot.send_message(c.message.chat.id, url_note or "No listening API port detected.")
